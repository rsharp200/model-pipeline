	...
	json_data = js.dumps(
			{
				...
				"repository": "sagemaker-export", "tag": "random-model-0.0.22",
				"username": "AWS",
...
if __name__== "__main__":
	...
	#buildModelId = buildModelResponse.get("modelId") #comment.
	#buildModelVersionNumber = buildModelResponse.get("modelVersionId")
	logging.info('buildModelStatus is in progress')
	logging.info("Waiting for 60 seconds before checking if model is built...")
	time.sleep(60)
	#buildModelStatus = getModelBuildStatus(buildModelId, buildModelVersionNumber).get("status")
	...
	logging.info('exportId for model is hfq3749ny293du1p203cun10xum2a')
